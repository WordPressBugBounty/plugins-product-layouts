...
class Layout7 extends Public_Render {
	...
	public function public_js() {
		wp_enqueue_style( 'wpte-caption-style7', WPTE_WPL_ASSETS . 'layouts/Caption/css/cube_effects.css', null, filemtime( WPTE_WPL_PATH . 'assets/layouts/Caption/css/cube_effects.css' ), false );
	}
	...
	public function layout_render( $settings, $user ) {
		...
		$cube_effect    = isset( $settings['wpte_product_layout_caption_style_7_effect'] ) ? $settings['wpte_product_layout_caption_style_7_effect'] : '';
		...
		if ( $query->have_posts() ) {
			...
			?>
			<div class="wpte-product-column wpte-product-caption-style-7-column <?php echo esc_attr( $column ); ?>">
			<?php
			while ( $query->have_posts() ) {
				...
				<div class="wpte-product-hover-style-caption wpte-swiper-slide">
					<div class="wpte-product-hover wpte-product-caption-hover wpte-product-caption-hover-style-7 wpte-product-cube-<?php echo esc_attr( $cube_effect ); ?>">
