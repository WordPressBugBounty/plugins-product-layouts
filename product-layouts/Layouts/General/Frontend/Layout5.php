...
class Layout5 extends Public_Render {
	...
	public function public_js() {
		wp_enqueue_style( 'wpte-general-style5', WPTE_WPL_ASSETS . 'layouts/General/css/layout5.css', null, filemtime( WPTE_WPL_PATH . 'assets/layouts/General/css/layout5.css' ), false );
	}
	...
	public function layout_render( $settings, $user ) {
		...
		$image_size       = isset( $settings['wpte_product_layout_general_style_5_image_size'] ) ? $settings['wpte_product_layout_general_style_5_image_size'] : '';
		...
		if ( $query->have_posts() ) {
			...
			while ( $query->have_posts() ) {
				...
				$hoverGimage           = $product_gallery_image ? 'wpte-general-layout-product-hover-g-image' : '';
				?>

				<div class="wpte-general-layout-row wpte-product-general-style-5 wpte-swiper-slide">
					<div class="wpte-general-layout-image-area">
						...
						if ( $showIcons ) :
							?>
						<div class="wpte-general-layout-product-icons wpte-general-layout-icon-left">
							<div class="wpte-general-layout-product-icons-inner">
								<div class="wpte-general-layout-icons wpte-general-layout-product-plus">
									<i class="wpte-icon icon-plus-3"></i>
								</div>
								...
								<div class="wpte-general-layout-icons wpte-general-layout-product-wishlist">
									<?php
									$wishlist_icon = $this->wpte_product_wishlist_render( get_the_ID(), 'icon', $settings, 'left' ); // $operator = icon, text, icontext // $IconPosition = top, right, bottom, left.
								...
								<div class="wpte-general-layout-icons wpte-general-layout-product-compare">
									<?php
									$cmopare_icon = $this->wpte_product_compare_render( get_the_ID(), 'icon', $settings, 'left' ); // $operator = icon, text, icontext // $IconPosition = top, right, bottom, left.
									echo wp_kses( (string) $cmopare_icon, wpte_allow_icons_html() );
									?>
								</div>
								...
								<div class="wpte-general-layout-icons wpte-general-layout-product-view wpte-product-layouts-quick-view">
									<?php
									$quickview_icon = $this->wpte_product_quickview_render( get_the_ID(), 'icon', $settings, 'left' ); // $operator = icon, text, icontext // $IconPosition = top, right, bottom, left.
									echo wp_kses( (string) $quickview_icon, wpte_allow_icons_html() );
									?>
								</div>
								<?php endif; if ( $is_cart_icon ) : ?>
								<div class="wpte-general-layout-icons wpte-general-layout-product-cart">
									<?php
									$this->wpte_product_cart_render( get_the_ID(), 'icon', $settings, 'left' ); // $operator = icon, text, icontext // $IconPosition = top, right, bottom, left.
									?>
								</div>
								<?php endif; ?>
							</div>
