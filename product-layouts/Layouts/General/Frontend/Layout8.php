...
class Layout8 extends Public_Render {
	...
	public function public_js() {
		wp_enqueue_style( 'wpte-general-style8', WPTE_WPL_ASSETS . 'layouts/General/css/layout8.css', null, filemtime( WPTE_WPL_PATH . 'assets/layouts/General/css/layout8.css' ), false );
	}
	...
	public function layout_render( $settings, $user ) {
		...
		$image_size       = isset( $settings['wpte_product_layout_general_style_8_image_size'] ) ? $settings['wpte_product_layout_general_style_8_image_size'] : '';
		...
		if ( $query->have_posts() ) {
			...
			?>
			<div class="wpte-product-column wpte-product-general-style-8-column <?php echo esc_attr( $column ); ?>">
			<?php
			while ( $query->have_posts() ) {
				...
				<div class="wpte-general-layout-row wpte-product-general-style-8 wpte-swiper-slide">
